...
import { applyMove, checkWinner, createEmptyBoard, getBoardKey } from "../utils/gameLogic";
...
export function useQLearning() {
    ...
    const [trainingStats, setTrainingStats] = useState<TrainingStats>({ wins: 0, losses: 0, draws: 0 });
    ...
    const playTrainingGame = useCallback((epsilon: number) => {
        ...
        while (true) {
            ...
            const action = agentRef.current.chooseAction(board, currentPlayer, epsilon);
            ...
            if (winner !== null) {
                for (let i = history.length - 1; i >= 0; i--) {
                    ...
                    const nextBoard = i < history.length - 1 ? applyMove(history[i].board, history[i].action, record.player) : null;
                    const nextState = nextBoard ? getBoardKey(nextBoard, record.player) : null;
        ...
        const trainAgent = useCallback(async (gamesNum: number, epsilon: number) => {
        setIsTraining(true);
    ...
    const getQValueForMove = useCallback((board: Board, position: number): number => {
